...
impl<'a, 'b> CodeNavigationContext<'a, 'b> {
    pub fn files_importing(
        ...
        source_sg
            ...
            .flat_map(|idx| {
                ...
                let active_token_text = &source_doc.content[token.start_byte..token.end_byte];
                all_docs
                    .iter()
                    ...
                    .filter(|doc| {
                        ...
                        scope_graph
                            ...
                            .any(|idx| match scope_graph.get_node(idx).unwrap() {
                                NodeKind::Import(n) => n.name(content) == active_token_text.as_bytes(),
                                _ => false,
                            })
                    })
                    .collect::<Vec<_>>()
    ...
    pub fn active_token_text(&self) -> &str {
        &self.source_document().content[self.active_token_range()]
    }
...
pub fn imported_ranges(
    ...
    source_sg
        ...
        .flat_map(|idx| {
            ...
            let active_token_text = &source_document.content[token.start_byte..token.end_byte];
            related_file_sg
                .graph
                .node_indices()
